...
-- Create a view that joins contact and contact_address to get the desired result
CREATE VIEW prisoner_contact_view AS
SELECT
    c.contact_id AS prisoner_contact_id,
    c.contact_id,
    c.last_name AS surname,
    c.first_name AS forename,
    c.middle_name,
    c.date_of_birth,
    '' AS relationship_code, -- Placeholder as relationship details are not present in the current schema
    '' AS relationship_description, -- Placeholder as relationship details are not present in the current schema
    CASE
        WHEN EXISTS (
            SELECT 1 FROM contact_address ca
            WHERE ca.contact_id = c.contact_id AND ca.primary_address = true
        ) THEN ARRAY(
            SELECT json_build_object(
                'contact_address_id', ca.contact_address_id,
                'flat', ca.flat,
                'property', ca.property,
                'street', ca.street,
                'area', ca.area,
                'city_code', ca.city_code,
                'county_code', ca.county_code,
                'post_code', ca.post_code,
                'country_code', ca.country_code,
                'primary_address', ca.primary_address
            ) FROM contact_address ca
            WHERE ca.contact_id = c.contact_id
        ) ELSE NULL
        END AS addresses,
    false AS approved_visitor, -- Placeholder as approved_visitor details are not present in the current schema
    pc.prisoner_number AS prisonerNumber,
    pc.active AS active
FROM
    contact c
        LEFT JOIN
    contact_address ca ON c.contact_id = ca.contact_id
WHERE
    ca.primary_address = true;

-- Grant permissions to access the view if necessary
-- GRANT SELECT ON prisoner_contact_view TO your_role;
