...
AS
  ...
  left join (select contact_id, contact_phone_id, phone_type, phone_number, row_number() over (partition by contact_id order by created_time desc) as rn
            from contact_phone) cp on (cp.contact_id = c.contact_id and cp.rn = 1)
--   left join (select distinct on (contact_id) contact_id, contact_phone_id, phone_type, phone_number
--             from contact_phone
--             order by contact_id, created_time desc) cp ON cp.contact_id = c.contact_id
