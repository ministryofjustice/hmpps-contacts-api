...
class CountryReferenceIntegrationTest : IntegrationTestBase() {
  ...
  inner class GetCountryByCountryId {
    ...
    private fun WebTestClient.getCountryReferenceData(url: String): MutableList<CountryReference> =
      get()
        .uri(url)
        .headers(setAuthorisation(roles = listOf("ROLE_CONTACTS_ADMIN")))
        .exchange()
        ...
        .expectBodyList(CountryReference::class.java)
  ...
  inner class GetAllCountries {
    ...
    fun `should return country reference data when get all countries`() {
      ...
      assertThat(countryReferences.contains(countryReference))
