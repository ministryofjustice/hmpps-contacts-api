...
class LanguageReferenceIntegrationTest : IntegrationTestBase() {
  ...
  inner class GetLanguageByLanguageId {
    ...
    private fun WebTestClient.getLanguageReferenceData(url: String): MutableList<LanguageReference> =
      get()
        .uri(url)
        .headers(setAuthorisation(roles = listOf("ROLE_CONTACTS_ADMIN")))
        .exchange()
        ...
        .expectBodyList(LanguageReference::class.java)
  ...
  inner class GetAllCountries {
    ...
    fun `should return language reference data when get all countries`() {
      ...
      val languageReference = LanguageReference(
        languageId = 226,
        nomisCode = "ZHA",
      ...
      assertThat(languageReferences.contains(languageReference))
