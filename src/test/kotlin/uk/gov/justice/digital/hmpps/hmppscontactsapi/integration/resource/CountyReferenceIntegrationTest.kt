...
class CountyReferenceIntegrationTest : IntegrationTestBase() {
  ...
  inner class GetCountyByCountyId {
    ...
    private fun WebTestClient.getCountyReferenceData(url: String): MutableList<CountyReference> =
      get()
        .uri(url)
        .headers(setAuthorisation(roles = listOf("ROLE_CONTACTS_ADMIN")))
        .exchange()
        ...
        .expectBodyList(CountyReference::class.java)
  ...
  inner class GetAllCountries {
    ...
    fun `should return county reference data when get all counties`() {
      ...
      val countyReference = CountyReference(
        countyId = 1L,
        nomisCode = "ZWE",
        nomisDescription = "Zimbabwe",
        displaySequence = 99,
      )
      assertThat(countyReferences.contains(countyReference))
