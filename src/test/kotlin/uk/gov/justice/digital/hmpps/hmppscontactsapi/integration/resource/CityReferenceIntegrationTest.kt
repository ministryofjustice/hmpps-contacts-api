...
class CityReferenceIntegrationTest : IntegrationTestBase() {
  ...
  inner class GetCityByCityId {
    ...
    private fun WebTestClient.getCityReferenceData(url: String): MutableList<CityReference> =
      get()
        .uri(url)
        .headers(setAuthorisation(roles = listOf("ROLE_CONTACTS_ADMIN")))
        .exchange()
        ...
        .expectBodyList(CityReference::class.java)
  ...
  inner class GetAllCities {
    ...
    fun `should return city reference data when get all cities`() {
      ...
      val cityReference = CityReference(
        cityId = 1L,
        nomisCode = "ZWE",
        nomisDescription = "Zimbabwe",
        displaySequence = 99,
      )
      assertThat(cityReferences.contains(cityReference))
